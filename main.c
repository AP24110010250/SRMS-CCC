...
/* Add prototype for local case-insensitive compare used earlier in the file */
...
void displayStudents(void) {
    ...
    if (!fp) {
        /* try fallback location where your attached file lives */
        fp = fopen("output\\student.txt", "r");
        if (!fp) {
            printf("No student records found (%s).\n", STUDENT_FILE);
            return;
        }
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strchr(line, '|')) {
            /* pipe-delimited format: roll|name|marks */
            ...
            tok = strtok(NULL, "|");
            if (!tok) continue;
            marks = (float) atof(tok);
        } else {
            /* space-separated format: roll <name possibly with spaces> marks
               Strategy: roll = first token, marks = last token, name = middle */
            char *start = line;
            while (isspace((unsigned char)*start)) start++;

            /* find first space after roll */
            char *p = start;
            while (*p && !isspace((unsigned char)*p)) p++;
            char *firstSpace = (*p) ? p : NULL;

            /* trim trailing whitespace */
            char *end = start + strlen(start);
            while (end > start && isspace((unsigned char)*(end-1))) end--;
            /* find last space before marks */
            char *lastSpace = end;
            while (lastSpace > start && !isspace((unsigned char)*(lastSpace-1))) lastSpace--;
            if (firstSpace && lastSpace && lastSpace > firstSpace) {
                /* roll */
                char rollbuf[32];
                size_t rlen = (size_t)(firstSpace - start);
                if (rlen >= sizeof(rollbuf)) rlen = sizeof(rollbuf)-1;
                strncpy(rollbuf, start, rlen);
                rollbuf[rlen] = '\0';
                roll = atoi(rollbuf);

                /* marks */
                size_t mlen = (size_t)(end - lastSpace);
                char marksbuf[64];
                if (mlen >= sizeof(marksbuf)) mlen = sizeof(marksbuf)-1;
                strncpy(marksbuf, lastSpace, mlen);
                marksbuf[mlen] = '\0';
                marks = (float) atof(marksbuf);

                /* name = between firstSpace+1 and lastSpace-1 */
                size_t nlen = (size_t)(lastSpace - (firstSpace + 1));
                if (nlen >= sizeof(name)) nlen = sizeof(name)-1;
                if (nlen > 0) {
                    strncpy(name, firstSpace + 1, nlen);
                    name[nlen] = '\0';
                } else {
                    name[0] = '\0';
                }
            } else {
                /* fallback: attempt simple sscanf */
                if (sscanf(line, "%d %199s %f", &roll, name, &marks) != 3) {
                    continue;
                }
            }
        }
...
void updateStudent(void) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (roll == key) {
            ...
            if ( (strcasecmp_local(newName, name) != 0) && nameExistsExceptRoll(newName, roll) ) {
...
/* ---------- portable case-insensitive compare (returns 0 if equal) ---------- */
int strcasecmp_local(const char *a, const char *b) {
    unsigned char ca, cb;
    while (*a || *b) {
